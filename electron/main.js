...
function createWindow() {
  ...
  // Position the BrowserView below the titlebar (48px) + body padding (16px)
  // Status bar (28px) is in the shell, so we subtract it to avoid overlap
  ...
  function updateViewBounds() {
    ...
    contentView.setBounds({
      ...
      height: height - TITLEBAR_HEIGHT - STATUS_BAR_HEIGHT - (BODY_PAD * 2) - 1,
    });
  }
...
function killFlask() {
  ...
  try {
    ...
    setTimeout(() => {
      if (flaskProcess) {
        try { flaskProcess.kill('SIGKILL'); } catch (_) {}
        flaskProcess = null;
      }
    }, 2000);
  } catch (_) {
